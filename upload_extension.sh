...
DevelopVersionString=`grep -E 'version.develop.*=' FYModuleB_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' FYModuleB_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' FYModuleB_Extension.podspec`
...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' FYModuleB_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" FYModuleB_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' FYModuleB_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" FYModuleB_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' FYModuleB_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" FYModuleB_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' FYModuleB_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' FYModuleB_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" FYModuleB_Extension.podspec
...
pod repo push ${Repo} FYModuleB_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
